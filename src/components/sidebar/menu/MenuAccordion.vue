...
import { cos } from '@amcharts/amcharts5/.internal/core/util/Math'
  ...
  onMounted(() => {
    pro.value=localStorage.getItem('userPermissions')
    pro.value = JSON.parse( pro.value);
  //   if(props.items)
    console.log(pro.value[0])
  //   accordionValue.value = props.items.map((item) => isItemExpanded(item))
     for(let i=0;i<props.items.length;i++){
       console.log(props.items[i])
       if(props.items[i].name == pro.value[0] || props.items[i].name == pro.value[1] ||props.items[i].name == pro.value[2] ||props.items[i].name == pro.value[3] ||props.items[i].name == pro.value[4] ||props.items[i].name == pro.value[5] || props.items[i].name == pro.value[6] || props.items[i].name == pro.value[7] ){
        arr.value.push(props.items[i])
       }
