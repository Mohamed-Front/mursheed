...
const deleteAction = () => {
  axios
    .delete(`/api/accommodition/${user.value.id}/delete`)
    .then((res) => {
      console.log(res.data)
      deleteDialog.value = false
      fetchData()
      toast.add({severity: 'success', summary: 'Successful', detail: 'accommodition deleted', life: 3000})
    })
    .catch(() => {
    })
        ...
        >
          <template #header>
            <div class="flex flex-column md:flex-row md:justify-between md:align-items-center">
              <h5 class="m-0">Manage Accommodation  </h5>
          ...
          <Column field="owner_info" header="Category" :sortable="true" header-style="width:14%; min-width:10rem;">
            <template #body="slotProps">
              {{ slotProps.data.category.name[langSelected] }}
            </template>
          </Column>
          ...
          <Column field="aval_status" header="Avalabilty" :sortable="true" header-style="width:14%; min-width:10rem;">
            <template #body="slotProps">
              <div class="m-auto text-center">
                <i class="pi pi-times-circle text-red-400 h-6" v-if="slotProps.data?.aval_status == 0"></i>
                  <i class="pi pi-check-circle text-green-500" v-if="slotProps.data?.aval_status == 1"></i>
              </div>

            </template>
          </Column>

          <Column field="info_status" header=" Owner information availability " :sortable="true"
                  header-style="width:14%; min-width:10rem;">
            <template #body="slotProps">
              <div class="m-auto text-center">
                <i class="pi pi-times-circle text-red-400 h-6" v-if="slotProps.data?.info_status == 0"></i>
                  <i class="pi pi-check-circle text-green-500" v-if="slotProps.data?.info_status == 1"></i>
              </div>
